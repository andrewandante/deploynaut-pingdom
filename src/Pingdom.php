...
/*
 * Interact with the Pingdom API: see https://www.pingdom.com/resources/api
 */
class Pingdom extends \Object
{
    ...
    protected function request($verb, $endpoint, $options = null)
    {
        $validVerbs = ['get', 'post', 'put', 'delete'];
        if (!in_array($verb, $validVerbs)) {
            throw new InvalidArgumentException(
                sprintf('Request verb must be one of: %s', implode(', ', $validVerbs)));
        }

        return $this->client->$verb($this->apiURL.$endpoint, [
            'auth' => [
                $this->getUsername(),
                $this->getPassword(),
            ],
